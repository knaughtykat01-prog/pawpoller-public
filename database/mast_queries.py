"""All SQL CRUD functions for the Mastodon (MAST) analytics database.
...
Key differences from other platforms:
  - submission_id is TEXT (ActivityPub URIs: https://instance/users/x/statuses/123)
  - Stats: likes (favourites), reposts (reblogs), replies. quotes is always 0
    (Mastodon has no native quote count) — kept for schema parity with bsky.
  - No individual comment tracking — just counts.
  - Content type: post / reply / quote / repost (boost you're tagged in).
"""
...
def upsert_mast_submission(conn: sqlite3.Connection, sub: dict, account_id: int) -> None:
    """Insert or update a Mastodon status's metadata and latest stats."""
    keywords_json = json.dumps(sub.get("keywords", []))
    # account_id set on INSERT only; the ON CONFLICT UPDATE leaves it alone.
    conn.execute(
        """INSERT INTO mast_submissions
...
def get_mast_submission(conn: sqlite3.Connection, submission_id: str) -> dict | None:
    row = conn.execute("SELECT * FROM mast_submissions WHERE submission_id = ?", (submission_id,)).fetchone()
    return dict(row) if row else None


def get_mast_submission_by_rkey(conn: sqlite3.Connection, rkey: str) -> dict | None:
    """Find a submission by rkey (last segment of the status URI)."""
    row = conn.execute(
        "SELECT * FROM mast_submissions WHERE submission_id LIKE ?",
        (f"%/{rkey}",),
    ).fetchone()
    return dict(row) if row else None


def get_all_mast_submissions(conn: sqlite3.Connection, sort_by: str = "likes", order: str = "desc", account_id: int | None = None) -> list[dict]:
    ...
    sql = "SELECT * FROM mast_submissions" + (f" WHERE {where}" if where else "")
...
def insert_mast_snapshot(conn: sqlite3.Connection, account_id: int, submission_id: str,
    ...
    conn.execute(
        "INSERT INTO mast_snapshots (account_id, submission_id, polled_at, likes, reposts, replies, quotes) VALUES (?, ?, ?, ?, ?, ?, ?)",
        (account_id, submission_id, ts, likes, reposts, replies, quotes),
    )


def get_mast_snapshots(conn: sqlite3.Connection, submission_id: str,
                        start: str | None = None, end: str | None = None) -> list[dict]:
    sql = "SELECT * FROM mast_snapshots WHERE submission_id = ?"
...
def get_mast_aggregate_snapshots(conn: sqlite3.Connection, start: str | None = None,
...
def get_mast_comparison_snapshots(conn: sqlite3.Connection, submission_ids: list[str],
    ...
    sql = f"SELECT * FROM mast_snapshots WHERE submission_id IN ({placeholders})"
...
def start_mast_poll_log(conn: sqlite3.Connection, account_id: int = 0) -> int:
    cur = conn.execute(
        "INSERT INTO mast_poll_log (started_at, status, account_id) VALUES (datetime('now'), 'running', ?)",
        (account_id,))
    conn.commit()
    return cur.lastrowid


def finish_mast_poll_log(conn: sqlite3.Connection, log_id: int, status: str,
                          submissions_found: int = 0, snapshots_inserted: int = 0,
                          error_message: str | None = None, duration_seconds: float = 0) -> None:
    conn.execute(
        """UPDATE mast_poll_log SET finished_at=datetime('now'), status=?, submissions_found=?,
...
def get_mast_last_poll(conn: sqlite3.Connection) -> dict | None:
    row = conn.execute("SELECT * FROM mast_poll_log ORDER BY started_at DESC LIMIT 1").fetchone()
    return dict(row) if row else None


def get_mast_poll_log(conn: sqlite3.Connection, limit: int = 50) -> list[dict]:
    rows = conn.execute("SELECT * FROM mast_poll_log ORDER BY started_at DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]


# -- MAST Summary ------------------------------------------------------------

def get_mast_summary(conn: sqlite3.Connection, account_id: int | None = None) -> dict:
    """Dashboard summary for Mastodon. With *account_id* set, the totals and
    ...
    totals = conn.execute(
        ...
        "FROM mast_submissions" + w,
        wp,
    ).fetchone()
    totals = dict(totals)

    top_liked = conn.execute(
        "SELECT submission_id, title, likes FROM mast_submissions" + w + " ORDER BY likes DESC LIMIT 5",
        wp,
    ).fetchall()

    top_reposted = conn.execute(
        "SELECT submission_id, title, reposts FROM mast_submissions" + w + " ORDER BY reposts DESC LIMIT 5",
        wp,
    ).fetchall()
    ...
    fastest_growing = conn.execute(
        """SELECT s.submission_id, s.title,
           ...
           FROM mast_submissions s
           LEFT JOIN (
               SELECT s1.submission_id, s1.likes, s1.reposts
               FROM mast_snapshots s1
               INNER JOIN (
...
def get_mast_growth_rates(conn: sqlite3.Connection) -> dict:
    totals = conn.execute(
        "SELECT COALESCE(SUM(likes),0) as likes, "
        "COALESCE(SUM(reposts),0) as reposts, "
        "COALESCE(SUM(replies),0) as replies "
        "FROM mast_submissions"
    ).fetchone()
    ...
    for label, hours in [("24h", 24), ("7d", 168), ("30d", 720)]:
        row = conn.execute(
            """SELECT SUM(likes) as likes, SUM(reposts) as reposts,
               ...
               FROM mast_snapshots WHERE polled_at = (
                   SELECT polled_at FROM mast_snapshots
...
def get_mast_submission_growth_rates(conn: sqlite3.Connection, submission_id: str) -> dict:
    sub = conn.execute(
        "SELECT likes, reposts, replies FROM mast_submissions WHERE submission_id = ?",
        (submission_id,),
    ).fetchone()
    if not sub:
        return {}
    ...
    for label, hours in [("24h", 24), ("7d", 168), ("30d", 720)]:
        row = conn.execute(
            """SELECT likes, reposts, replies
               FROM mast_snapshots WHERE submission_id = ? AND polled_at <= datetime('now', ? || ' hours')
               ORDER BY polled_at DESC LIMIT 1""",
...
def get_mast_submission_deltas(conn: sqlite3.Connection) -> dict[str, dict]:
    rows = conn.execute(
