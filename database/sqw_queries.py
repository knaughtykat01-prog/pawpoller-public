...
def get_all_sqw_submissions(conn: sqlite3.Connection, sort_by: str = "views", order: str = "desc", account_id: int | None = None) -> list[dict]:
    ...
    sql = "SELECT * FROM sqw_submissions" + (f" WHERE {where}" if where else "")
...
def get_sqw_summary(conn: sqlite3.Connection, account_id: int | None = None) -> dict:
    ...
    totals = conn.execute(
        ...
        "FROM sqw_submissions" + w,
        wp,
    ).fetchone()
    totals = dict(totals)

    top_viewed = conn.execute(
        "SELECT submission_id, title, views FROM sqw_submissions" + w + " ORDER BY views DESC LIMIT 5",
        wp,
    ).fetchall()

    top_faved = conn.execute(
        "SELECT submission_id, title, favorites_count FROM sqw_submissions" + w + " ORDER BY favorites_count DESC LIMIT 5",
        wp,
    ).fetchall()
