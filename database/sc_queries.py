"""SQL CRUD for the SoundCloud (`sc`) analytics database — MEDIAPLATS §3 (4.22.0).

Poll+post audio platform. Standard engagement triple — views (plays) /
favorites_count (likes) / comments_count — so this mirrors the gallery query
modules (fn/fa/ws/…). `submission_id` is the SoundCloud track id as TEXT; the
row also carries the urn, genre, tag list, sharing, reposts and downloads.
"""
...
def upsert_sc_submission(conn: sqlite3.Connection, sub: dict, account_id: int) -> None:
    conn.execute(
        """INSERT INTO sc_submissions
           (submission_id, account_id, urn, title, description, genre, tag_list, sharing,
            username, link, artwork_url, duration_ms, posted_at,
            views, favorites_count, comments_count, reposts_count, downloads_count, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
           ON CONFLICT(submission_id) DO UPDATE SET
            urn=excluded.urn, title=excluded.title, description=excluded.description,
            genre=excluded.genre, tag_list=excluded.tag_list, sharing=excluded.sharing,
            username=excluded.username, link=excluded.link, artwork_url=excluded.artwork_url,
            duration_ms=excluded.duration_ms, posted_at=COALESCE(excluded.posted_at, sc_submissions.posted_at),
            ...
            downloads_count=excluded.downloads_count, updated_at=datetime('now')
        """,
        (
            sub["submission_id"], account_id, sub.get("urn", ""), sub.get("title", ""),
            sub.get("description", ""), sub.get("genre", ""), sub.get("tag_list", ""),
            sub.get("sharing", ""), sub.get("username", ""), sub.get("link", ""),
            sub.get("artwork_url", ""), sub.get("duration_ms", 0), sub.get("posted_at") or None,
            sub.get("views", 0), sub.get("favorites_count", 0), sub.get("comments_count", 0),
            sub.get("reposts_count", 0), sub.get("downloads_count", 0),
        ),
    )


def get_sc_submission(conn: sqlite3.Connection, submission_id: str) -> dict | None:
    row = conn.execute("SELECT * FROM sc_submissions WHERE submission_id = ?",
                       (submission_id,)).fetchone()
    return dict(row) if row else None


def get_all_sc_submissions(conn: sqlite3.Connection, sort_by: str = "views",
                           order: str = "desc", account_id: int | None = None) -> list[dict]:
    allowed_sorts = {"views", "favorites_count", "comments_count", "reposts_count",
                     "title", "posted_at", "updated_at"}
    if sort_by not in allowed_sorts:
        sort_by = "views"
    order_dir = "DESC" if order.lower() == "desc" else "ASC"
    where, params = account_clause(account_id)
    sql = "SELECT * FROM sc_submissions" + (f" WHERE {where}" if where else "")
...
def insert_sc_snapshot(conn: sqlite3.Connection, account_id: int, submission_id: str,
    ...
    conn.execute(
        "INSERT INTO sc_snapshots (account_id, submission_id, polled_at, views, "
        "favorites_count, comments_count) VALUES (?, ?, ?, ?, ?, ?)",
        (account_id, submission_id, ts, views, favorites_count, comments_count),
    )


def get_sc_snapshots(conn: sqlite3.Connection, submission_id: str,
                     start: str | None = None, end: str | None = None) -> list[dict]:
    sql = "SELECT * FROM sc_snapshots WHERE submission_id = ?"
...
def get_sc_aggregate_snapshots(conn: sqlite3.Connection, start: str | None = None,
    ...
    sql = ("SELECT polled_at, SUM(views) as views, SUM(favorites_count) as favorites_count, "
           "SUM(comments_count) as comments_count FROM sc_snapshots")
    params: list[Any] = []
    conditions = []
    if start:
...
def get_sc_comparison_snapshots(conn: sqlite3.Connection, submission_ids: list[str],
    ...
    sql = f"SELECT * FROM sc_snapshots WHERE submission_id IN ({placeholders})"
...
def start_sc_poll_log(conn: sqlite3.Connection, account_id: int = 0) -> int:
    cur = conn.execute(
        "INSERT INTO sc_poll_log (started_at, status, account_id) "
        "VALUES (datetime('now'), 'running', ?)", (account_id,))
    conn.commit()
    return cur.lastrowid


def finish_sc_poll_log(conn: sqlite3.Connection, log_id: int, status: str,
                       submissions_found: int = 0, snapshots_inserted: int = 0,
                       error_message: str | None = None, duration_seconds: float = 0) -> None:
    conn.execute(
        """UPDATE sc_poll_log SET finished_at=datetime('now'), status=?, submissions_found=?,
...
def get_sc_last_poll(conn: sqlite3.Connection) -> dict | None:
    row = conn.execute("SELECT * FROM sc_poll_log ORDER BY started_at DESC LIMIT 1").fetchone()
    return dict(row) if row else None


def get_sc_poll_log(conn: sqlite3.Connection, limit: int = 50) -> list[dict]:
    rows = conn.execute("SELECT * FROM sc_poll_log ORDER BY started_at DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]


# -- Summary -----------------------------------------------------------------

def get_sc_summary(conn: sqlite3.Connection, account_id: int | None = None) -> dict:
    ...
    totals = dict(conn.execute(
        ...
        "COALESCE(SUM(comments_count),0) as total_comments FROM sc_submissions" + w, wp,
    ).fetchone())

    top_viewed = conn.execute(
        "SELECT submission_id, title, views FROM sc_submissions" + w + " ORDER BY views DESC LIMIT 5", wp,
    ).fetchall()
    top_faved = conn.execute(
        "SELECT submission_id, title, favorites_count FROM sc_submissions" + w
...
def get_sc_growth_rates(conn: sqlite3.Connection) -> dict:
    totals = conn.execute(
        "SELECT COALESCE(SUM(views),0) as views, COALESCE(SUM(favorites_count),0) as favorites_count, "
        "COALESCE(SUM(comments_count),0) as comments_count FROM sc_submissions"
    ).fetchone()
    rates = {}
    for label, hours in [("24h", 24), ("7d", 168), ("30d", 720)]:
        row = conn.execute(
            """SELECT SUM(views) as views, SUM(favorites_count) as favorites_count,
               ...
               FROM sc_snapshots WHERE polled_at = (
                   SELECT polled_at FROM sc_snapshots
...
def get_sc_submission_growth_rates(conn: sqlite3.Connection, submission_id: str) -> dict:
    sub = conn.execute(
        "SELECT views, favorites_count, comments_count FROM sc_submissions WHERE submission_id = ?",
        (submission_id,),
    ).fetchone()
    if not sub:
        return {}
    ...
    for label, hours in [("24h", 24), ("7d", 168), ("30d", 720)]:
        row = conn.execute(
            """SELECT views, favorites_count, comments_count FROM sc_snapshots
...
def get_sc_submission_deltas(conn: sqlite3.Connection) -> dict[str, dict]:
    rows = conn.execute(
