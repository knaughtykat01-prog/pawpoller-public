"""SQL CRUD for the YouTube (`yt`) analytics database — MEDIAPLATS §6 (4.24.0).

Poll+post video platform. Standard engagement triple — views / favorites_count
(likes) / comments_count — so this mirrors the gallery query modules
(sc/fn/fa/…). `submission_id` is the YouTube video id; the row also carries the
tags, category, privacy and upload status.
"""
...
def upsert_yt_submission(conn: sqlite3.Connection, sub: dict, account_id: int) -> None:
    tags = sub.get("tags", "")
    if isinstance(tags, (list, tuple)):
        tags = ", ".join(str(t) for t in tags)
    conn.execute(
        """INSERT INTO yt_submissions
           (submission_id, account_id, title, description, tags, category_id, privacy, upload_status,
            username, link, thumbnail_url, duration, posted_at,
           ...
           ON CONFLICT(submission_id) DO UPDATE SET
            ...
            category_id=excluded.category_id, privacy=excluded.privacy, upload_status=excluded.upload_status,
            username=excluded.username, link=excluded.link, thumbnail_url=excluded.thumbnail_url,
            duration=excluded.duration, posted_at=COALESCE(NULLIF(excluded.posted_at, ''), yt_submissions.posted_at),
        ...
        """,
        (
            sub["submission_id"], account_id, sub.get("title", ""), sub.get("description", ""), tags,
            sub.get("category_id", ""), sub.get("privacy", ""), sub.get("upload_status", ""),
            sub.get("username", ""), sub.get("link", ""), sub.get("thumbnail_url", ""), sub.get("duration", ""),
            sub.get("posted_at") or None,
            sub.get("views", 0), sub.get("favorites_count", 0), sub.get("comments_count", 0),
        ),
    )


def get_yt_submission(conn: sqlite3.Connection, submission_id: str) -> dict | None:
    row = conn.execute("SELECT * FROM yt_submissions WHERE submission_id = ?",
                       (submission_id,)).fetchone()
    return dict(row) if row else None


def get_all_yt_submissions(conn: sqlite3.Connection, sort_by: str = "views",
    ...
    sql = "SELECT * FROM yt_submissions" + (f" WHERE {where}" if where else "")
...
def insert_yt_snapshot(conn: sqlite3.Connection, account_id: int, submission_id: str,
    ...
    conn.execute(
        "INSERT INTO yt_snapshots (account_id, submission_id, polled_at, views, "
        "favorites_count, comments_count) VALUES (?, ?, ?, ?, ?, ?)",
        (account_id, submission_id, ts, views, favorites_count, comments_count),
    )


def get_yt_snapshots(conn: sqlite3.Connection, submission_id: str,
                     start: str | None = None, end: str | None = None) -> list[dict]:
    sql = "SELECT * FROM yt_snapshots WHERE submission_id = ?"
...
def get_yt_aggregate_snapshots(conn: sqlite3.Connection, start: str | None = None,
    ...
    sql = ("SELECT polled_at, SUM(views) as views, SUM(favorites_count) as favorites_count, "
           "SUM(comments_count) as comments_count FROM yt_snapshots")
    params: list[Any] = []
    conditions = []
    if start:
...
def get_yt_comparison_snapshots(conn: sqlite3.Connection, submission_ids: list[str],
    ...
    sql = f"SELECT * FROM yt_snapshots WHERE submission_id IN ({placeholders})"
...
def start_yt_poll_log(conn: sqlite3.Connection, account_id: int = 0) -> int:
    cur = conn.execute(
        "INSERT INTO yt_poll_log (started_at, status, account_id) "
        "VALUES (datetime('now'), 'running', ?)", (account_id,))
    conn.commit()
    return cur.lastrowid


def finish_yt_poll_log(conn: sqlite3.Connection, log_id: int, status: str,
                       submissions_found: int = 0, snapshots_inserted: int = 0,
                       error_message: str | None = None, duration_seconds: float = 0) -> None:
    conn.execute(
        """UPDATE yt_poll_log SET finished_at=datetime('now'), status=?, submissions_found=?,
...
def get_yt_last_poll(conn: sqlite3.Connection) -> dict | None:
    row = conn.execute("SELECT * FROM yt_poll_log ORDER BY started_at DESC LIMIT 1").fetchone()
    return dict(row) if row else None


def get_yt_poll_log(conn: sqlite3.Connection, limit: int = 50) -> list[dict]:
    rows = conn.execute("SELECT * FROM yt_poll_log ORDER BY started_at DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]


# -- Summary -----------------------------------------------------------------

def get_yt_summary(conn: sqlite3.Connection, account_id: int | None = None) -> dict:
    ...
    totals = dict(conn.execute(
        ...
        "COALESCE(SUM(comments_count),0) as total_comments FROM yt_submissions" + w, wp,
    ).fetchone())

    top_viewed = conn.execute(
        "SELECT submission_id, title, views FROM yt_submissions" + w + " ORDER BY views DESC LIMIT 5", wp,
    ).fetchall()
    top_faved = conn.execute(
        "SELECT submission_id, title, favorites_count FROM yt_submissions" + w
...
def get_yt_growth_rates(conn: sqlite3.Connection) -> dict:
    totals = conn.execute(
        "SELECT COALESCE(SUM(views),0) as views, COALESCE(SUM(favorites_count),0) as favorites_count, "
        "COALESCE(SUM(comments_count),0) as comments_count FROM yt_submissions"
    ).fetchone()
    rates = {}
    for label, hours in [("24h", 24), ("7d", 168), ("30d", 720)]:
        row = conn.execute(
            """SELECT SUM(views) as views, SUM(favorites_count) as favorites_count,
               ...
               FROM yt_snapshots WHERE polled_at = (
                   SELECT polled_at FROM yt_snapshots
...
def get_yt_submission_growth_rates(conn: sqlite3.Connection, submission_id: str) -> dict:
    sub = conn.execute(
        "SELECT views, favorites_count, comments_count FROM yt_submissions WHERE submission_id = ?",
        (submission_id,),
    ).fetchone()
    if not sub:
        return {}
    ...
    for label, hours in [("24h", 24), ("7d", 168), ("30d", 720)]:
        row = conn.execute(
            """SELECT views, favorites_count, comments_count FROM yt_snapshots
...
def get_yt_submission_deltas(conn: sqlite3.Connection) -> dict[str, dict]:
    rows = conn.execute(
