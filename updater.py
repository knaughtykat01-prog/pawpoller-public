...
def spawn_relauncher() -> None:
    ...
    if sys.platform.startswith("linux"):
        ...
        import shlex
        # An AppImage runs from a READ-ONLY squashfs mount (4.34.1, RESTARTAPPIMG).
        # `build-appimage.sh`'s AppRun execs `${HERE}/usr/bin/PawPoller/PawPoller`, so
        # `sys.executable` — and therefore `exe.parent` — is inside the mount, and
        # writing the relaunch script there raised OSError: Read-only file system. The
        # Restart button simply could not work on a Linux AppImage.
        #
        # Two things have to change together. `$APPIMAGE` is the path to the .AppImage
        # FILE, which is what must be re-executed — relaunching the inner binary would
        # start it outside its own mount, without the runtime that put it there. And the
        # script goes somewhere writable, since the install directory is not.
        _appimage = os.environ.get("APPIMAGE")
        _target = Path(_appimage) if _appimage else exe
        _dir = exe.parent
        if _appimage or not os.access(_dir, os.W_OK):
            # ⚠ NOT the shared temp dir (4.34.4). This script is written, chmod'd
            # 0755 and executed, and /tmp/_relaunch.sh is a predictable name any
            # local user can pre-create as a symlink — write_text follows one, so
            # we would overwrite and mark executable whatever it points at.
            # fs.protected_symlinks defaults to 1 on current kernels, but relying
            # on a sysctl we never set is not a defence. APPDATA_DIR is
            # user-owned and always writable, which is why the fallback exists.
            _dir = Path(config.APPDATA_DIR)
            _dir.mkdir(parents=True, exist_ok=True)
        _exe = shlex.quote(str(_target))
        script = _dir / "_relaunch.sh"
